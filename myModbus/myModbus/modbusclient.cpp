...
std::string HOST;
mymodbus::mymodbus(std::string host, uint16_t port = 502)
{
    HOST = host;
    PORT = port;
    _slaveid = 1;
 ...
 */
mymodbus::~mymodbus(void) = default;

/**
 * Modbus Slave ID Setter
 * @param id  ID of the Modbus Server Slave
 */
void mymodbus::modbus_set_slave_id(int id)
{
    _slaveid = id;
}
...
bool mymodbus::modbus_connect()
{
    ...
    iResult = getaddrinfo(HOST.c_str(), ststr.str().c_str(), &hints, &result);
    if (iResult != 0) {
        printf("getaddrinfo failed with error: %d\n", iResult);
        WSACleanup();
        LOG("ERR GetAddrinfo() failed ");
        _connected = false;
        return _connected;
    }
    ...
    if (_socket == INVALID_SOCKET) {
        printf("Unable to connect to ModBus server!\n");
        WSACleanup();
...
//    int iRet = InetPton(AF_UNSPEC, (PCWSTR)HOST.c_str(), &_server.sin_addr.s_addr);
...
//        WSACleanup();
//#endif
//        std::cout << "Err connection " << std::to_string(GetLastError()) << std::endl;
//        return false;
//    }
 ...
 */
 void mymodbus::modbus_close() const
{
 ...
 */
 void mymodbus::modbus_build_request(uint8_t * to_send, uint16_t address, int func) const
{
 ...
 */
 int mymodbus::modbus_write(uint16_t address, uint16_t amount, int func, const uint16_t * value)
{
    int status = 0;
 ...
 * @param func      Modbus Functional Code
 */
 int mymodbus::modbus_read(uint16_t address, uint16_t amount, int func)
{
    uint8_t to_send[12];
    modbus_build_request(to_send, address, func);
    to_send[5] = 6;
 ...
 */
 int mymodbus::modbus_read_holding_registers(uint16_t address, uint16_t amount, uint16_t * buffer)
{
    if (_connected)
    {
 ...
 * @param buffer      Buffer to Store Data Read from Registers
 */
 int mymodbus::modbus_read_input_registers(uint16_t address, uint16_t amount, uint16_t * buffer)
{
    if (_connected)
    {
 ...
 */
 int mymodbus::modbus_read_coils(uint16_t address, uint16_t amount, bool* buffer)
{
    if (_connected)
    {
        if (amount > 2040)
 ...
 */
 int mymodbus::modbus_read_input_bits(uint16_t address, uint16_t amount, bool* buffer)
{
    if (_connected)
    {
        if (amount > 2040)
 ...
 */
 int mymodbus::modbus_write_coil(uint16_t address, const bool& to_write)
{
    if (_connected)
    {
 ...
 */
 int mymodbus::modbus_write_register(uint16_t address, const uint16_t & value)
{
    if (_connected)
    {
 ...
 */
 int mymodbus::modbus_write_coils(uint16_t address, uint16_t amount, const bool* value)
{
    if (_connected)
    {
 ...
 int mymodbus::modbus_read_registers(uint16_t address, uint16_t amount, uint16_t * buffer)
{
    if (_connected)
    {
        modbus_read(address, amount, READ_INPUT_REGS);
...
std::string mymodbus::GetErr(void)
{
    return error_msg;
}
 ...
 */
 int mymodbus::modbus_write_registers(uint16_t address, uint16_t amount, const uint16_t * value)
{
    if (_connected)
    {
 ...
 */
 ssize_t mymodbus::modbus_send(uint8_t * to_send, size_t length)
{
    _msg_id++;
    return (ssize_t)send(_socket, (const char*)to_send, (int)length, 0);
}
 ...
 */
 ssize_t mymodbus::modbus_receive(uint8_t * buffer) const
{
    return recv(_socket, (char*)buffer, MAX_MSG_LENGTH, 0);
}

 void mymodbus::set_bad_con()
{
    err = true;
    error_msg = "BAD CONNECTION";
}

 void mymodbus::set_bad_input()
{
    err = true;
 ...
 * @param func  Modbus Functional Code
 */
 void mymodbus::modbuserror_handle(const uint8_t * msg, int func)
{
    err = false;
    error_msg = "NO ERR";
    if (msg[7] == func + 0x80)
